...
} from '/var/task/.gemini/antigravity/scratch/pdf-editor/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '/var/task/.gemini/antigravity/scratch/pdf-editor/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('/var/task/.gemini/antigravity/scratch/pdf-editor')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '/var/task/.gemini/antigravity/scratch/pdf-editor',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('/var/task/.gemini/antigravity/scratch/pdf-editor/.netlify/dist/run/handlers/server.js')
